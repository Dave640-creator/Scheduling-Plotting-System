...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt->execute([$d['course_code'],$d['course_title'],(int)$d['year_level'],$d['semester_type'],$lecUnits,$labUnits,$d['category']??'major']);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
        ...
        // Lecture/lab units, year level, and semester are exactly the fields
        // an existing schedule's validity depends on (required weekly hours,
        // which component is even allowed, which section year level it can
        // be plotted for). If any of these change while schedules already
        // reference this course, those schedules could silently become
        // inconsistent with the (now-edited) course record. Block the edit
        // instead -- Option A from the bug report -- and ask for the
        // schedules to be dealt with first.
        $structuralChanged =
            abs((float)$current['lec_units'] - $lecUnits) > 0.0001 ||
            abs((float)$current['lab_units'] - $labUnits) > 0.0001 ||
            (int)$current['year_level'] !== (int)$d['year_level'] ||
        ...
        if ($structuralChanged) {
            ...
            if ((int)$countStmt->fetchColumn() > 0) {
                json_response(false, 'Cannot change lecture/laboratory units, year level, or semester for this course because it already has plotted schedules. Delete or update those schedules first, then edit the course.', null, 422);
            }
        }
        ...
        $stmt->execute([$d['course_code'],$d['course_title'],(int)$d['year_level'],$d['semester_type'],$lecUnits,$labUnits,$d['category']??'major',$id]);
